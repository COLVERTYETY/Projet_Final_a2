...
namespace decouverte
{
    public class MyImage
    {
        ...
        public MyImage(int _width, int _height)
        ...
        public MyImage GreyScale()
        {
            ...
            for(int i = 0 ; i < this.height ; i++) {
                for (int j = 0; j < this.width ; j++) {
                    res.data[j, i] = new pixel((byte)this.data[j, i].avg, (byte)this.data[j, i].avg, (byte)this.data[j, i].avg);
                }
            }
            return res;
        }
        ...
        public MyImage Mirror(bool vertical = true) {
            ...
            for(int i = 0 ; i < this.height ; i++) {
                for (int j = 0; j < this.width ; j++) {
                    if (vertical) {
                        res.data[j, i] = this.data[this.width-1-j, i];
                    }
                    else {
                        res.data[j, i] = this.data[j, this.height-1-i];
                    }
                }
            }
            return res;
